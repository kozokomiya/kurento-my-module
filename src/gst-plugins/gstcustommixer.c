...
#define PLUGIN_NAME "custommixer"

#define KMS_CUSTOM_MIXER_LOCK(mixer) \
  (g_rec_mutex_lock (&( (KmsCustomMixer *) (mixer))->priv->mutex))

#define KMS_CUSTOM_MIXER_UNLOCK(mixer) \
  (g_rec_mutex_unlock (&( (KmsCustomMixer *) (mixer))->priv->mutex))

GST_DEBUG_CATEGORY_STATIC (kms_custom_mixer_debug_category);
#define GST_CAT_DEFAULT kms_custom_mixer_debug_category

#define KMS_CUSTOM_MIXER_GET_PRIVATE(obj) (\
  G_TYPE_INSTANCE_GET_PRIVATE (               \
    (obj),                                    \
    KMS_TYPE_CUSTOM_MIXER,                 \
    KmsCustomMixerPrivate                  \
  )                                           \
)
...
G_DEFINE_TYPE_WITH_CODE (KmsCustomMixer, kms_custom_mixer,
    KMS_TYPE_BASE_HUB,
    GST_DEBUG_CATEGORY_INIT (kms_custom_mixer_debug_category, PLUGIN_NAME,
        0, "debug category for custommixer element"));

typedef struct _KmsCustomMixerData
{
  KmsRefStruct parent;
  gint id;
  KmsCustomMixer *mixer;
...
#define KMS_CUSTOM_MIXER_REF(data) \
  kms_ref_struct_ref (KMS_REF_STRUCT_CAST (data))
#define KMS_CUSTOM_MIXER_UNREF(data) \
  kms_ref_struct_unref (KMS_REF_STRUCT_CAST (data))

static void
kms_destroy_custom_mixer_data (KmsCustomMixerData * data)
{
  g_slice_free (KmsCustomMixerData, data);
}

static KmsCustomMixerData *
kms_create_custom_mixer_data ()
{
  KmsCustomMixerData *data;

  data = g_slice_new0 (KmsCustomMixerData);
  kms_ref_struct_init (KMS_REF_STRUCT_CAST (data),
      (GDestroyNotify) kms_destroy_custom_mixer_data);
...
{
  KmsCustomMixerData *port_data_a = (KmsCustomMixerData *) a;
  KmsCustomMixerData *port_data_b = (KmsCustomMixerData *) b;
...
static gboolean
remove_elements_from_pipeline (KmsCustomMixerData * port_data)
{
  KmsCustomMixer *self = port_data->mixer;

  KMS_CUSTOM_MIXER_LOCK (self);
...
{
  KmsCustomMixerData *port_data = (KmsCustomMixerData *) data;
  KmsCustomMixer *self = port_data->mixer;
  GstEvent *event;
  ...
  kms_loop_idle_add_full (self->priv->loop, G_PRIORITY_DEFAULT,
      ...
      KMS_CUSTOM_MIXER_REF (port_data),
...
static void
kms_custom_mixer_port_data_destroy (gpointer data)
{
  KmsCustomMixerData *port_data = (KmsCustomMixerData *) data;
  KmsCustomMixer *self = port_data->mixer;
  GstPad *audiosink;
  ...
  if (port_data->input) {
    ...
    if (!GST_OBJECT_FLAG_IS_SET (pad, GST_PAD_FLAG_EOS)) {
      ...
      if (port_data->input && self->priv->n_elems > 0) {
        ...
        self->priv->n_elems--;
        kms_custom_mixer_recalculate_sizes (self);
      }
      KMS_CUSTOM_MIXER_UNLOCK (self);
    ...
    KmsCustomMixerData * data)
{
  ...
  mixer = KMS_CUSTOM_MIXER (data->mixer);
  GST_DEBUG ("stream start detected %d", data->id);
  KMS_CUSTOM_MIXER_LOCK (mixer);
  ...
  data->probe_id = gst_pad_add_probe (data->video_mixer_pad,
      ...
      KMS_CUSTOM_MIXER_REF (data), (GDestroyNotify) kms_ref_struct_unref);
  ...
  mixer->priv->n_elems++;
  kms_custom_mixer_recalculate_sizes (mixer);
...
static void
kms_custom_mixer_unhandle_port (KmsBaseHub * mixer, gint id)
{
  KmsCustomMixer *self = KMS_CUSTOM_MIXER (mixer);
  ...
  KMS_BASE_HUB_CLASS (G_OBJECT_CLASS
      (kms_custom_mixer_parent_class))->unhandle_port (mixer, id);
}

static KmsCustomMixerData *
kms_custom_mixer_port_data_create (KmsCustomMixer * mixer, gint id)
{
  KmsCustomMixerData *data;
  GstPad *tee_src;
  GstCaps *filtercaps;
  ...
  data = kms_create_custom_mixer_data ();
  data->mixer = mixer;
  data->id = id;
  data->input = FALSE;
  data->removing = FALSE;
...
{
  KmsCustomMixer *self = KMS_CUSTOM_MIXER (mixer);
  KmsCustomMixerData *port_data;
  gint port_id;

  GST_DEBUG ("Handle new HubPort");
  port_id = KMS_BASE_HUB_CLASS (G_OBJECT_CLASS
      (kms_custom_mixer_parent_class))->handle_port (mixer, mixer_end_point);
  ...
  port_data = kms_custom_mixer_port_data_create (self, port_id);
...
static void
kms_custom_mixer_dispose (GObject * object)
{
  KmsCustomMixer *self = KMS_CUSTOM_MIXER (object);
  ...
  G_OBJECT_CLASS (kms_custom_mixer_parent_class)->dispose (object);
}

static void
kms_custom_mixer_finalize (GObject * object)
{
  KmsCustomMixer *self = KMS_CUSTOM_MIXER (object);
  ...
  G_OBJECT_CLASS (kms_custom_mixer_parent_class)->finalize (object);
}

static void
kms_custom_mixer_class_init (KmsCustomMixerClass * klass)
{
  ...
  gst_element_class_set_static_metadata (GST_ELEMENT_CLASS (klass),
      "CustomMixer", "Generic", "Mixer element that composes n input flows"
      " in one output flow", "David Fernandez <d.fernandezlop@gmail.com>");

  gobject_class->dispose = GST_DEBUG_FUNCPTR (kms_custom_mixer_dispose);
  gobject_class->finalize = GST_DEBUG_FUNCPTR (kms_custom_mixer_finalize);

  base_hub_class->handle_port =
      GST_DEBUG_FUNCPTR (kms_custom_mixer_handle_port);
  base_hub_class->unhandle_port =
      GST_DEBUG_FUNCPTR (kms_custom_mixer_unhandle_port);
  ...
  g_type_class_add_private (klass, sizeof (KmsCustomMixerPrivate));
}

static void
kms_custom_mixer_init (KmsCustomMixer * self)
{
  self->priv = KMS_CUSTOM_MIXER_GET_PRIVATE (self);

  g_rec_mutex_init (&self->priv->mutex);

  self->priv->ports = g_hash_table_new_full (g_int_hash, g_int_equal,
      release_gint, kms_custom_mixer_port_data_destroy);
...
gboolean
kms_custom_mixer_plugin_init (GstPlugin * plugin)
{
